...
/**
 * Während dieser Phase findet eine Auswahl der Trumpffarbe statt. Zum Beginn der Phase bekommt jeder Spieler 5
 * karten. Danach findet die Auswahl statt. In ersten zwei Zügen wird entschieden, ob die vorgeschlagene Farbe
 * gespielt wird. Danach kann jeder Spieler seinen Wunsch spielen. Wenn nach 4 Versuchen der Trumpf doch nicht
 * ausgewählt wird, werden die Karten neu verteilt und die Runde beginnt von vorne.
 */
public class Wahl extends AbstractPhase implements Phase {
    ...
    private void validateTrumpfAuswahl(Karte trumpfAuswahl) {
        ...
        if (anzTrumpfwahlAbsagen <= nativeTrumpVersuche && isNotNativeFarbe(trumpfAuswahl)) {
            throw new IllegalArgumentException("Erste zwei Versuche entscheiden über den vorgeschlagenen Trumpf");
        }
    }
