...
public class PartieMapper {
    ...
    public static PartieDto mapPartieToDto(Partie partie) {
        ...
        runde.setSpielerKarten(partie.getSpielerKarten().stream().map(KarteMapper::mapKarteToDto).toList());
        runde.setGegnerKarten(partie.getGegnerKarten().stream().map(KarteMapper::mapKarteToDto).toList());
